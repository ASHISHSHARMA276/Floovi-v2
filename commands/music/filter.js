  ...
  async execute(client, message, args) {
    ...
    if (!player) {
      return message.channel.send({ 
        embeds: [new EmbedBuilder().setColor("#ff0000").setDescription("❌ No music is currently playing.")] 
      });
    }
    ...
    collector.on("collect", async (i) => {
      ...
      if (selected === "off") {
        ...
        await i.update({ 
          embeds: [new EmbedBuilder().setColor("#5865F2").setDescription("✅ All filters have been cleared.")], 
          components: [] 
        });
      } else {
        ...
        await i.update({ 
          embeds: [new EmbedBuilder().setColor("#5865F2").setDescription(`✅ Applied **${selected}** filter.`)], 
          components: [] 
        });
      }
    });
